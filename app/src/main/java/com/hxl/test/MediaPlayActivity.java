...
/**
 * Created by huaxianlian on 2017/8/25
 */
...
public class MediaPlayActivity extends Activity implements SurfaceHolder.Callback{
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if(TextUtils.isEmpty(m_strFile)){
            m_strFile = m_strNextFile2;
        }
        m_Activity = this;
    ...
    public void surfaceCreated(SurfaceHolder surfaceholder)
    {
        ...
        if (m_Player != null)
        {
            ...
            if (m_Player.getDuration() <= 0)
                m_Player.setParam(PlayerParameter.QCPLAY_PID_Disable_Video, 0, null);
            else
                m_Player.play();
...
//            m_Player.onVideoSizeChanged ();
    ...
    public int onEvent(int nID, int nArg1, int nArg2, Object obj) {
        if (nID == PlayerParameter.QC_MSG_PLAY_OPEN_DONE) {

            //	m_Player.SetParam(PlayerParameter.QCPLAY_PID_SendOut_AudioBuff, 0, null);
            //	m_Player.SetParam(PlayerParameter.QCPLAY_PID_SendOut_VideoBuff, 0, null);

            m_nDuration = (int)m_Player.getDuration();
            m_Player.play();
        }
        else if (nID == PlayerParameter.QC_MSG_SNKV_NEW_FORMAT) {
            if (nArg1 == 0 || nArg2 == 0)
                return 0;
            if (nArg1 > nArg2) {
        ...
        else if (nID == PlayerParameter.QC_MSG_PLAY_DURATION)
            m_nDuration = (int)m_Player.getDuration();
        else if (nID == PlayerParameter.QC_MSG_PLAY_OPEN_FAILED) {
        ...
        else if (nID == PlayerParameter.QC_MSG_PLAY_COMPLETE)
            Close ();
        else if (nID == PlayerParameter.QC_MSG_PLAY_SEEK_DONE || nID == PlayerParameter.QC_MSG_PLAY_SEEK_FAILED) {
            if (m_dlgWait != null) {
                m_dlgWait.cancel();
                m_dlgWait = null;
            }
        }
        else if (nID == PlayerParameter.QC_MSG_HTTP_DISCONNECTED) {
            if (m_dlgOK != null)
                return 0;
            m_dlgOK = new AlertDialog.Builder(this);
            m_dlgOK.setTitle("Error");
        ...
        else if (nID == PlayerParameter.QC_MSG_HTTP_RECONNECT_FAILED) {
            if (m_dlgOK != null)
                return 0;
            m_dlgOK = new AlertDialog.Builder(this);
            m_dlgOK.setTitle("Error");
        ...
        else if (nID == PlayerParameter.QC_MSG_HTTP_RECONNECT_SUCESS) {
            return 0;
        }
        else if (nID == PlayerParameter.QC_MSG_SNKV_FIRST_FRAME) {
            if (m_dlgWait != null) {
                m_dlgWait.cancel();
                m_dlgWait = null;
            }
        }
        else if (nID == PlayerParameter.QC_MSG_RTMP_METADATA) {
...
//			Log.v("PlayerView  MetaData *** ", strMetaData);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (nAction) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (m_Player != null) {
                    if (m_nPlayIndex == 0)
                        m_Player.open(m_strNextFile1, PlayerParameter.QCPLAY_OPEN_SAME_SOURCE);
                    else if (m_nPlayIndex == 1)
                        m_Player.open(m_strNextFile2, PlayerParameter.QCPLAY_OPEN_SAME_SOURCE);
                    else if (m_nPlayIndex == 2)
                        m_Player.open(m_strNextFile3, PlayerParameter.QCPLAY_OPEN_SAME_SOURCE);
                    else if (m_nPlayIndex == 3)
                        m_Player.open(m_strNextFile4, PlayerParameter.QCPLAY_OPEN_SAME_SOURCE);
                    else if (m_nPlayIndex == 4)
                        m_Player.open(m_strNextFile5, PlayerParameter.QCPLAY_OPEN_SAME_SOURCE);
                }
                m_nPlayIndex++;
                if (m_nPlayIndex >= 5)
                    m_nPlayIndex = 0;
                //  m_Player.SetParam (PlayerParameter.QCPLAY_PID_Capture_Image, 0, null);
                return true;
    ...
    protected void onPause()
    {
        ...
        {
            if (m_Player.getDuration() <= 0) {
                m_Player.setParam(PlayerParameter.QCPLAY_PID_Disable_Video, 1, null);
            }  else {
    ...
    private void OpenFile (String strPath) {
        ...
        int	nFlag = 0;
        if (nVideoDec == 3)
            nFlag = PlayerParameter.QCPLAY_OPEN_VIDDEC_HW;
        ...
        m_Player.setView(m_svVideo);
        nRet = (int) m_Player.init(apkPath, nFlag);
        if (nRet != 0) {
            Close ();
            return;
        }
...
//        m_Player.setEventListener(this);
        ...
        //m_Player.SetParam(PlayerParameter.QCPLAY_PID_Speed, 0, byKeyText);

        if (nDownloadFile > 0) {
            m_Player.setParam(PlayerParameter.QCPLAY_PID_Prefer_Protocol, PlayerParameter.QC_IOPROTOCOL_HTTPPD, null);
            File file = Environment.getExternalStorageDirectory();
            String strPDPath = file.getPath() + "/QPlayer/PDFile";
            m_Player.setParam(PlayerParameter.QCPLAY_PID_PD_Save_Path, 0, strPDPath);
        }
    ...
    private void initControls (){
        ...
        m_btnPause.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                //if (m_Player != null) {
                //	m_Player.open(m_strFile, PlayerParameter.QCPLAY_OPEN_SAME_SOURCE);
                //  m_Player.SetParam (PlayerParameter.QCPLAY_PID_Capture_Image, 0, null);
                //	return;
                //}
                if (m_Player != null)
